...
module MonthReportsHelper
  ...
  def indentification(_projection, pdf)
  ...
  def indentification_right(_projection, pdf)
    ...
    pdf.text "<b>Fator de Conversão em:</b> #{@start_date.end_of_month.strftime('%d/%m/%Y')} #{last_rate.presence || '-'}", :inline_format => true   
  end
  
  ## B - Esquema de Liberação de Recursos
  
  def schema_b(_projection, pdf)
  ...
  def exercicio_1(_protection, pdf)
    ...
    pdf.table(data, :width => 540, :row_colors => %w[E9ECEF FFFFFF FFFFFF])
  end
  
  ## 1 - A Realizado no Exercício
  
  def exercicio_2(_projection, pdf)
  ...
  def exercicio_3(_projection, pdf)
  ...
  def exercicio_4(_projection, pdf)
  ...
  def saldo_devedor(_projection, pdf)
    total_principal = 0 
    total_principal_brl = 0 
    total_interest_brl = 0 
    total_charges_brl = 0 
