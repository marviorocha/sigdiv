...
const Currencies = () => {
  ...
  useEffect(() => {
    ...
    return (
        ...
        <tr>
          <td key={item.attributes.id} >{item.attributes.name}</td>
          <td>{item.attributes.formula}</td>
          <td>{item.attributes.description}</td>
          <td>{item.attributes.last_currency}</td>
          <td>
            {item.attributes.date_currency
              ? dayjs(item.attributes.date_currency).format("DD/MM/YYYY")
              : ""}
          </td>
  ...
  const spinnel = <div className="center"><i className="fa fa-spinner fa-pulse fa-3x fa-fw"></i></div>;
